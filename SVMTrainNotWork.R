...
col=16
Twb.type<-cbind(Twb.prof[,1:col],ptype.fac) %>% as.data.frame
if(col==16){colnames(Twb.type)<-c("H0","H1","H2", "H3","H4","H5","H6","H7", "H8","H9","H10","H11","H12","H13","H14","H15","H16","ptype.df")}
if(col==31){colnames(Twb.type)<-c("H0","H1","H2", "H3","H4","H5","H6","H7", "H8","H9","H10","H11","H12","H13","H14","H15","H16","H17","H18","H19","H20","H21","H22","H23","H24","H25","H26","H27","H28","H29","H30","ptype.df")}
attach(Twb.type)
...
for(i in 1:1){
  ...
  #implement the SVM
  model[[i]]<- svm( ptype.df~., data=Twb.type[train.rows,], probability=T, type='C-classification',kernel='sigmoid', class.weights=c("1"=t.w[1], "2"=t.w[2], "3"=t.w[3], "4"=t.w[4]))
  res[[i]] <- predict( model[[i]], newdata=Twb.type[test.rows,])
  zz[,,i]<-table(pred = res[[i]], true = Twb.type[test.rows,ncol(Twb.type)])
}
save(zz, file='tablesmall.RData')
save(model, file='model1small.RData')
detach(Twb.type)
